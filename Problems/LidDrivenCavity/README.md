Instructions
=====

The neutronic parameters (neutron flux, mean neutron generation time, delayed fractions and decay constants for the DNP groups) are identical to those used in [3]

# Step 1.1 


# Step 2.1 

# References

[3] J. Groth-Jensen, A. Nalbandyan, E.B. Klinkby, B. Lauritzen, P. Sabbagh, A.V. Pedersen,
Verification of multiphysics coupling techniques for modeling of molten salt reactors,
Annals of Nuclear Energy,
Volume 164,
2021,
108578,
ISSN 0306-4549,
https://doi.org/10.1016/j.anucene.2021.108578.
